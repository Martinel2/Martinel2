...
* **CI/CD 배포 자동화 및 배포 환경 트러블슈팅**
   * GitHub Actions 기반 CI/CD 최초 구축 과정에서 CI 단계의 테스트 DB 종속성 문제와 CD 배포 단계의 컨테이너 연결 실패 등 복합적인 장애 발생
   * 배포 스크립트와 에러 로그 분석 및 84번에 걸친 시행착오
      * **CI 환경 격리 -** 테스트 전용 프로파일 설정 및 **H2 인메모리 DB**를 도입하여 테스트 독립성 확보
      * **CD 의존성 제어** - 단순 네트워크 에러가 아닌, WAS와 DB 컨테이너 간의 **구동 타이밍 불일치**임을 파악하고 Docker Compose의 `depends_on` 옵션으로 실행 순서를 강제 제어

### [쇼핑몰 웹사이트](https://github.com/Martinel2/SpringProject_Shoppingmall) / 개인프로젝트 / 2024.03 ~ 2024.09
- **비즈니스 정책 기반의 ERD 재설계 및 장바구니/결제 데이터 정합성 확보**
    - 판매자가 상품을 삭제할 때 타 사용자의 장바구니(Cart)에 해당 상품이 담겨 있어 FK(외래키) 제약 조건 위반으로 삭제가 거부되는 데이터 무결성 충돌 발생
    - **해결**
        - **도메인 정책 수립 (판매자 권리 우선)**
            - 쇼핑몰 정책상 상품 관리의 주체는 판매자여야 함을 정의
            - 상품 삭제 시 이를 참조하는 장바구니 데이터를 선제적으로 일괄 삭제하도록 비즈니스 로직 재구축
        - **결제/장바구니 테이블 단건 분할 (확장성 고려)**
            - 다중 결제 시 배열이 아닌 상품 수(N개)만큼 Row를 나누어 적재
            - 장바구니 비우기를 Bulk Delete 대신 단건 반복 삭제로 구현
                - 미래의 '일부 상품 선택 결제' 및 '개별 상품 배송/취소' 요구사항에 유연하게 대응할 수 있는 아키텍처 설계
    - **결과**
